...
SemaphoreHandle_t plan_buffer_line_mutex = NULL;
...
//在没有匀速段的情况下，根据初速度，末速度，加速度和距离计算减速点的位置
...
//根据下一个block的entry_speed修正当前block的entry_speed
...
void plan_init(void) 
{
  ...
  previous_nominal_speed = 0.0;
	...
	plan_buffer_line_mutex=xSemaphoreCreateMutex();
}
...
void plan_buffer_line(const float x, const float y, const float z, const float e, float feed_rate, const uint8_t extruder)
{
  ...
  // Bail if this is a zero-length block
	...
	//如果最长的步长都小于最小步长，则返回，不动
  if (block->step_event_count <= dropsegments)
  { 
		jmp_config_state_struct.n_motion_command_sum++;
		xSemaphoreGive(plan_buffer_line_mutex);
    return; 
  }
  #ifdef DEBUG_PRINTF 
	...
	if(jmp_config_state_struct.breakpoint_front_reading==1)
	{
		u32 command_num;
		command_num=jmp_config_state_struct.motion_command_sum-jmp_config_state_struct.n_motion_command_sum;
		if(command_num==jmp_breakpoint_save_struct.command_num)//如果已经到了断点的指令
		{
			jmp_config_state_struct.breakpoint_front_reading=0;//下面可以继续执行了
			jmp_breakpoint_recover_postion();//恢复断点的位置
			//重新计算block的各轴步数
			block->steps_x = labs(target[X_AXIS]-position[X_AXIS]);
			block->steps_y = labs(target[Y_AXIS]-position[Y_AXIS]);
			block->steps_z = labs(target[Z_AXIS]-position[Z_AXIS]);
  ...
  // Calculate and limit speed in mm/sec for each axis
	//当有一轴超出限定值，其他各轴也一同受到限制，通过一个speed_factor因子控制
...
// block_manage_put_line(x,y,z,e,feed_rate);
...
//抛弃剩下的block
void plan_discard_next_all_block(void)
{
	if(block_buffer_head != block_buffer_tail)
	{
		block_buffer_head = (block_buffer_tail + 1) & (BLOCK_BUFFER_SIZE - 1);
	}
}
