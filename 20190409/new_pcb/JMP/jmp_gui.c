...
void jmp_gui_start_print(void)
{
	...
	strcpy((char*)jmp_storage_read_filepath,(const char*)&jmp_gui_gcode_file_name[jmp_gui_gcode_file_name_p+jmp_gui_gcode_file_name_chosen][0]);//复制文件名
				...
				case GUI_PRINT_MAIN://打印主界面
				{
					if(jmp_guiuart_rx_str.command==0x83)
					{
						...
						else if(jmp_guiuart_rx_str.start_address==GUI_PRINT_MAIN_BREAKPOINT_TEST)//断点测试
						{
							...
							plan_buffer_line(0,0,0,0,10,active_extruder);
							while(blocks_queued())
							{
								vTaskDelay(10);
							}
