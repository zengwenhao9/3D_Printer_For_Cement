...
u8 jmp_breakpoint_save_breakpoint(void)
{
	...
	//获取断点参数
	jmp_breakpoint_save_struct.command_num=jmp_config_state_struct.current_command_num;
	for(i=0;i<NUM_AXIS;i++)
	{
		jmp_breakpoint_save_struct.count_position[i]=count_position[i];
	}
	for(i=0;i<jmp_storage_read_filepath_sum;i++)
	{
		jmp_breakpoint_save_struct.file_name[i]=jmp_storage_read_filepath[i];
	}
	//保存断点
