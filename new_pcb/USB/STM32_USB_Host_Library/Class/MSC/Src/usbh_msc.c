    ...
    if(MSC_Handle->current_lun < MSC_Handle->max_lun)
    {
      ...
      case MSC_READ_CAPACITY10:   
        ...
        if(scsi_status == USBH_OK)
        {
          ...
          {
            //¼ÆËãUÅÌÈÝÁ¿
            UdiskCap=(uint64_t)(MSC_Handle->unit[MSC_Handle->current_lun].capacity.block_nbr)*(uint64_t)(MSC_Handle->unit[MSC_Handle->current_lun].capacity.block_size);	
            USBH_UsrLog ("MSC Device capacity : %d MBytes", (uint32_t)(UdiskCap>>20));
