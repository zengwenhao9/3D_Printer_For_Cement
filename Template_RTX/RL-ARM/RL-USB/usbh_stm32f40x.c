 ...
 *----------------------------------------------------------------------------*/
/*
*  1. 这个函数被修改，修改后用于V5和V6板子的OTG_FS接口PA11和PA12。
*  2. OTG_FS_ID引脚未使用，此引脚用户识别是Host还是Device。
*  3. OTG_FS_PS用于开关对外供电。
*/
BOOL USBH_STM32_Pins_Config (U8 ctrl, BOOL on) {
  if (!ctrl) {
    ...
    //usbh_stm32_uncfg_pin (PORT_H,  5);        /* PH5:  reset mode  modified by eric2013 */
    //usbh_stm32_uncfg_pin (PORT_A, 10);        /* PA10: reset mode  modified by eric2013 */
