...
//*src:源地址
//n:需要复制的内存长度(字节为单位)
void tmmemcpy(void *des,void *src,u32 n)  
{  
    u8 *xdes=des;
	u8 *xsrc=src; 
    while(n--)*xdes++=*xsrc++;  
...
void tmmemset(void *s,u8 c,u32 count)  
{  
    u8 *xs = s;  
    while(count--)*xs++=c;  
}	
//内存管理初始化  
//memx:所属内存块
void tm_mem_init(u8 memx)  
{  
    tmmemset(mallco_dev.memmap[memx],0,memtblsize[memx]*4);	//内存状态表数据清零  
 	mallco_dev.memrdy[memx]=1;								//内存管理初始化OK  
}  
//获取内存使用率
//memx:所属内存块
//返回值:使用率(扩大了10倍,0~1000,代表0.0%~100.0%)
u16 tm_mem_perused(u8 memx)  
{  
    u32 used=0;  
    u32 i;  
...
u32 tm_mem_malloc(u8 memx,u32 size)  
{  
...
//offset:内存地址偏移
//返回值:0,释放成功;1,释放失败;  
u8 tm_mem_free(u8 memx,u32 offset)  
{  
    int i;  
...
//memx:所属内存块
//ptr:内存首地址 
void tmfree(u8 memx,void *ptr)  
{  
	u32 offset;   
	if(ptr==NULL)return;//地址为0.  
 	offset=(u32)ptr-(u32)mallco_dev.membase[memx];     
    tm_mem_free(memx,offset);	//释放内存      
}  
//分配内存(外部调用)
//memx:所属内存块
//size:内存大小(字节)
//返回值:分配到的内存首地址.
void *tmmalloc(u8 memx,u32 size)  
{  
    u32 offset;   
	offset=tm_mem_malloc(memx,size);  	   	 	   
...
//size:要分配的内存大小(字节)
//返回值:新分配到的内存首地址.
void *tmrealloc(u8 memx,void *ptr,u32 size)  
{  
    u32 offset;    
    offset=tm_mem_malloc(memx,size);   	
    if(offset==0XFFFFFFFF)return NULL;     
    ...
    {  									   
	    tmmemcpy((void*)((u32)mallco_dev.membase[memx]+offset),ptr,size);	//拷贝旧内存内容到新内存   
        tmfree(memx,ptr);  											  		//释放旧内存
