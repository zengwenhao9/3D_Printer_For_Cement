...
u8 FTL_Init(void)
{
	...
	if(nand_dev.lut)tmfree(SRAMEX,nand_dev.lut);
	nand_dev.lut=tmmalloc(SRAMEX,(nand_dev.block_totalnum)*2); 	//给LUT表申请内存
...
u32 FTL_SearchBadBlock(void)
{
	...
	u32 i,j; 
	u32 goodblock=0;
	blktbl=tmmalloc(SRAMEX,nand_dev.block_totalnum);//申请block坏块表内存,对应项:0,好块;1,坏块;
	NAND_EraseChip(); 						//全片擦除
