...
u8 jmp_breakpoint_save_breakpoint(void)
{
	...
	//保存断点
	bp_result = f_open(&bp_file,  "0:/breakpoint.bin", FA_CREATE_ALWAYS | FA_WRITE);
	if(bp_result!=FR_OK)
	{
		f_close(&bp_file);
		return 0;
...
u8 jmp_breakpoint_check_breakpoint(void)
{
	...
	while(1)//通过文件名判断
	{
		...
		if(strstr(bp_FileInf.fname,"breakpoint.bin")!=NULL)
		{
			...
			jmp_breakpoint_read_breakpoint();
			{
				...
				while(1)
				{
					...
					if(strstr(bp_FileInf.fname,jmp_breakpoint_save_struct.file_name)!=NULL)
					{
						f_closedir(&bp_DirInf);
						return 1;
					}
...
void jmp_breakpoint_read_breakpoint(void)
{
	...
	//读取断点信息
	bp_result=f_open(&bp_file, "0:/breakpoint.bin", FA_OPEN_EXISTING | FA_READ);
...
void jmp_breakpoint_go_breakpoint(void)
{
	...
	//运动到断点，E轴不动
	plan_buffer_line(0, 0, z, 0, BREAKPOINT_RETURN_FEEDRATE, active_extruder);
