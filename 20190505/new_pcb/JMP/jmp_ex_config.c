...
/***********************************************************************************************************
该模块为外部配置模块，用于通过外部文件配置系统参数
***********************************************************************************************************/


JMPExCofigStruct jmp_ex_config_struct;//外部配置结构体
...
u8 jmp_ex_config_file_buf[JMP_EX_CONFIG_FILE_BUF_SUM];//数据缓冲区
u8 jmp_ex_config_file_cut_buf[JMP_EX_CONFIG_FILE_BUF_SUM];//截取数据后的缓冲区
u8 jmp_ex_config_check_digit_buff[JMP_EX_CONFIG_CHECK_DIGIT_BUF_SUM];//数字缓冲区
...
u8 jmp_ex_config_read_param(const char* str,double *param)
{
	...
	u32 length;
	char* segment;
	segment=strstr((const char*)jmp_ex_config_file_buf,str);//获取对应字段
	if(segment==NULL)
	{
		return 0;
	}
...
u8 jmp_ex_config_read_and_set(void)
{
	...
	ex_result=f_open(&ex_file, "2:/config.txt", FA_OPEN_EXISTING | FA_READ);//打开内部配置文件
	file_size=f_size(&ex_file);
	jmp_ex_config_file_buf_clear();
	ex_result=f_read(&ex_file, &jmp_ex_config_file_buf, file_size, &bw);//读取数据
...
{
	if(jmp_ex_config_check_file(0)==1)//检测内部有没有配置文件
	{
		if(jmp_ex_config_read_and_set()==0)//配置
		{
			jmp_ex_config_default_set();//默认配置
		}
	}
	else
	{
...
{
	u32 bw;
	ex_result = f_open(&ex_file,  "2:/config.txt", FA_CREATE_ALWAYS | FA_WRITE);
