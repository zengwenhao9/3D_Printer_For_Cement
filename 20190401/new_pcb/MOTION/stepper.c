...
volatile long count_direction[NUM_AXIS] = { 1, 1, 1, 1};
...
void st_timer_interrupt(void)
{
    ...
    if (step_events_completed >= current_block->step_event_count) 
		{
			if(jmp_config_state_struct.block_manage_stop==1)
			{
				block_manage_adjust_current_block();
			}
			current_block = NULL;
      plan_discard_current_block();
			if(jmp_config_state_struct.block_manage_stop==1)
			{
				jmp_config_state_struct.block_manage_stop=0;
				block_manage_save_position_function();
				block_manage_save_planner_position_function();
				block_manage_adjust_planner_position();
			}
//			block_manage_discard_current_line();
    }   
  } 
}
